...
duration_classifier = pd.DataFrame([[range(30, 91), 5],
                                    [range(91, 181), 4],
                                    [range(181, 241), 3],
                                    [range(241, 361), 2],
                                    [range(361, 721), 1]], columns = ['Dur', 'Mark'])
...
for i in range(days):   #номер дня это i+1
    if (i+1) % 7 == 6 or (i+1) % 7 == 0: #проверка на субботу - воскресенье
        continue
    ...
    for n in range(int(randnumapps)):
        ...
        duration = random.choice(range(30, 721, 15))
        try:
        ...
        if approvement == 'Approved':
            ...
            endQ = round(beginQ * (1 + interest_rate * (endDate - beginDate) / 365), 2)
    ...
    for n in range(int(randnumdeposits)):
        ...
        clientId = id + 1
        beginDate = i + 1
        endDate = i + 1 + random.choice(range(30, 735, 15))
        if endDate % 7 == 6:
                endDate = endDate + 2
        if endDate % 7 == 0:
                endDate = endDate + 1
        beginQ = random.choice(range(1, 501))*1000
        endQ = round(beginQ * (1 + 0.03 * (endDate - beginDate) / 365), 2)
        status = 'Active'
...
#        print('lol')
    ...
    if cash() < 0:
        ...
        endQ = round(beginQ * (1 + risk_free_rate * (endDate - beginDate) / 365), 2)
        status = 'Active'
        pd.DataFrame([{'AccType':accType, 'ClientId':clientId, 'BeginDate':beginDate, 'EndDate':endDate, 'BeginQ':beginQ, 'EndQ':endQ, 'Status':status}]).to_sql('deposit_accounts', con = engine, index = False, if_exists = 'append')
        id += 1
        liabilities += beginQ
