#Geschaftsbank - Stage4. Backup 30.01.2019

#Результаты потрясающие! Годовые ставки приведены к годовой размерности: прибыль резко сократилась, чего и следовало ожидать
#Скрин в разделе testing
...
for i in range(days):   #номер дня это i+1
    ...
    for n in range(int(randnumloans)):
        ...
        endDate = i + 1 + random.choice(range(1, 6)) * 5
    ...
    for n in range(int(randnumdeposits)):
        ...
        endDate = i + 1 + random.choice(range(1, 6)) * 5
        beginQ = random.choice(range(1, 101))*100
        endQ = beginQ * (1 + 0.03 * (endDate - beginDate) / 252)
        status = 'Active'
        newcostomer = [accType, clientId, beginDate, endDate, beginQ, endQ, status]
...
#вечер
    ...
    if cash() < 0:
        ...
        endQ = beginQ * (1 + 0.01 * (endDate - beginDate) / 252)
        status = 'Active'
        newcostomer = [accType, clientId, beginDate, endDate, beginQ, endQ, status]
        open_deposit_accounts.loc[len(open_deposit_accounts)] = newcostomer
        id += 1
...
incomestatement = pd.DataFrame([interestincome, interestcosts, 
    ...
    index = ['Interest income', 'Interest costs (-)', 'Net profit margin', 'Net reserve expenses(-)',
             'NPM after reserves', 'Operating costs (-)', 'Default losses (-)', 'Net income'], 
    columns = ['Accounts'])
